...
const emit = defineEmits<{ 'update:modelValue': [boolean]; scanQr: [] }>()

const items = [
  { label: 'Dati profilo', icon: 'user' },
  { label: 'Assistenza', icon: 'help' },
  { label: 'Team di collaboratori', icon: 'team' },
  { label: 'Gestione avvisi', icon: 'bell' }
]
    ...
    <div v-if="modelValue" class="menu-overlay" @mousedown.self="close">
      <Transition name="menu-panel" appear>
        <aside v-if="modelValue" class="menu-panel" role="dialog" aria-modal="true" aria-label="Menu">
          ...
          <nav class="menu-list">
            <button v-for="item in items" :key="item.label" type="button" class="menu-item">
              ...
              <svg v-else width="18" height="18" viewBox="0 0 20 20" fill="none"><path d="M10 2a5 5 0 00-5 5v3l-1.5 3h13L15 10V7a5 5 0 00-5-5zM8 16a2 2 0 004 0" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" /></svg>
              <span>{{ item.label }}</span>
            </button>
          </nav>

          <div class="menu-footer">
            <Button variant="secondary" @click="emit('scanQr')">
